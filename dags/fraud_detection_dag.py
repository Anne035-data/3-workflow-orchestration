...
def process_transaction(**context):
    ...
    try:
        ...
        with mlflow.start_run(run_name="prediction"):
            ...
            mlflow.log_metric("transaction_amount", float(df['amt'].iloc[0]))
            
            # Log des paramètres
            mlflow.log_param("transaction_id", df['trans_num'])
            mlflow.log_param("merchant", df['merchant'])
            
            # Log du résultat
            mlflow.log_metric("is_fraud", int(prediction))
